...
public class Wies implements Serializable,Comparable<Wies> {
	...
	private int getWiesCompareValue() {
		...
		if (isSameValue()) {
			sameFourOfValuePoints = 100 * cards.get(0).getFourOfValueWiesPoints();
		} else {
		...
		return sameFamilyInRow > sameFourOfValuePoints ? sameFamilyInRow : sameFourOfValuePoints;
	}
	...
	public static List<Wies> getPossibleWies(User u) {
		...
		int[][] cardArray = new int[CardValue.values().length][CardFamily.values().length];
		...
		for(int i =0 ; i < CardValue.values().length; i++){
			if(cardArray[i][0]+cardArray[i][1]+cardArray[i][2]+cardArray[i][3] == CardFamily.values().length){
		...
		for(int f =0 ; f < CardFamily.values().length; f++){
			for(int v =0 ; v < CardValue.values().length; v++){
				...
				if(rowCount>3){
					wies.add(getWiesOfSameFamilyInRow(allCards,u,f+1,v+6-rowCount,rowCount));
				}
	...
	private static Wies getWiesOfSameFamilyInRow(List<Card> allCards,User u,int family, int firstValue, int rowCount) {
		List<Card> wiesCard = new ArrayList<Card>();
		for(Card c : allCards){
			if(c.getOrderValue() >= firstValue && c.getOrderValue() < firstValue + rowCount && c.getFamilyOrder() == family ){
				wiesCard.add(c);
			}
		}
